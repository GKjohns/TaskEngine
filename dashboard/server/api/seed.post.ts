...
const SEED_DATA = [
    ...
    nodes: [
      {
        id: 'load_inventory',
        type: 'retrieve',
        description: 'Load the most recent inventory snapshot generated since the last successful run.',
        per_artifact: false,
        depends_on: [],
        prompt: null,
        labels: null,
        ...
        filter: null,
        retrieve_config: {
          match: 'Green Bean Inventory',
          task_id: null,
          time_window: 'since_last_run',
          content_search: null,
          types: ['csv'],
          limit: 1,
          sort: 'newest'
  ...
  {
    plan: {
      title: 'Daily Exchange Rate Snapshot',
      description: 'Fetches current exchange rates from a public API and turns them into a concise operations-ready note.',
      prompt: 'Fetch the latest USD exchange rates for EUR, GBP, and CAD from a public API. Summarize the latest values and note whether anything looks materially different from normal day-to-day movement.'
    },
    task: {
      title: 'Daily Exchange Rate Check',
      prompt: 'Pull the latest USD exchange rates for EUR, GBP, and CAD. Summarize the values in a short note for the operations team so they can quickly see the current baseline.',
      trigger_type: 'scheduled' as const,
      schedule_config: { cron: '0 6 * * 1-5', description: 'Weekdays at 6am' }
    },
    nodes: [
      {
        id: 'fetch_rates',
        type: 'http_fetch',
        description: 'Fetch the latest exchange rates from the public API.',
        per_artifact: false,
        depends_on: [],
        prompt: null,
        labels: null,
        ...
        url: 'https://api.exchangerate.host/latest?base=USD&symbols=EUR,GBP,CAD',
        method: 'GET',
        headers: null,
        body: null,
        response_type: 'json',
        artifact_title: 'Exchange Rates Snapshot',
        condition: null,
        if_true_node: null,
        if_false_node: null,
        duration: null,
      ...
      },
      {
        id: 'summarize_rates',
        type: 'llm_summarize',
        description: 'Summarize the fetched exchange rates into a short operations note.',
        per_artifact: false,
        depends_on: ['fetch_rates'],
        prompt: 'Read the fetched exchange-rate payload and produce a short note with the latest USD exchange rates for EUR, GBP, and CAD. Keep it factual and easy to scan.',
        max_length: 300,
        labels: null,
        source: null,
        filter: null,
      ...
      },
      {
        id: 'output',
        type: 'emit',
        description: 'Save the exchange-rate snapshot.',
        per_artifact: false,
        depends_on: ['summarize_rates'],
        title: 'Exchange Rate Snapshot — {{date}}',
        format: 'markdown',
        prompt: null,
        labels: null,
        max_length: null,
    ...
    nodes: [
      {
        id: 'load_latest_pnl',
        type: 'retrieve',
        description: 'Load the most recent monthly P&L artifact created since the last successful run.',
        per_artifact: false,
        depends_on: [],
        prompt: null,
        labels: null,
  ...
  },
  {
    ...
    nodes: [
      {
        id: 'load_maintenance_log',
        type: 'retrieve',
        description: 'Load the most recent maintenance log created since the last successful run.',
        per_artifact: false,
        depends_on: [],
        prompt: null,
        labels: null,
        ...
        filter: null,
        retrieve_config: {
          match: 'Equipment Maintenance Log',
          task_id: null,
          time_window: 'since_last_run',
          content_search: null,
          types: null,
      ...
      },
      {
        ...
        per_artifact: false,
        depends_on: ['load_maintenance_log'],
